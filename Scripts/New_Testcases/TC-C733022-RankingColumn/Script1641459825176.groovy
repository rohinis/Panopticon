...
try {
    ...
    //Verify the new calculated column should appear in the Table
    TestObject rankingcolumn = WebUI.modifyObjectProperty(findTestObject('Object Repository/DataTable/Calculated_column'), 
        'text', 'contains', 'Ranking', true)

    boolean newcol = WebUI.verifyElementPresent(rankingcolumn, 5, FailureHandling.CONTINUE_ON_FAILURE)

    if (newcol) {
        extentTest.log(LogStatus.PASS, 'Verify the new ranking column should appear in the Table::' + newcol)
    } else {
    ...
    //Go Back to Visualization part and create a BAR Graph
    ...
    TestObject draggableobject1 = WebUI.modifyObjectProperty(findTestObject('Object Repository/General_Button'), 'text', 
        'equals', 'Ranking', true)

    TestObject dropobject1 = WebUI.modifyObjectProperty(findTestObject('Object Repository/General_Button'), 'text', 'equals', 
        'Color', true)
